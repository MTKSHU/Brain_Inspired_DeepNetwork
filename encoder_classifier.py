...
trainset_autoencoder = AVA_Dataset_All(root_dir=path + "images/resized_images_320_320_padded_white/", transform=transform)
trainset_len_autoencoder = trainset_autoencoder.__len__()
print(trainset_autoencoder[0][0].shape)
trainloader_autoencoder = torch.utils.data.DataLoader(trainset_autoencoder, batch_size=BatchSize, shuffle=True,
                                                      num_workers=10)
...
for epoch in range(iterations):
    ...
    for i, data in enumerate(trainloader_autoencoder, 0):
        # if i % 10 == 0:
        #     print("batch :",i)
        # get the inputs
        inputs = data
    ...
    trainLoss.append(runningLoss / (trainset_len_autoencoder / BatchSize))
    # if i % 100 == 99:    # print every 2000 mini-batches
    #     print('[%d, %5d] loss: %.6f' %
    ...
    model_path = "models/" + \
                 str(epoch + 1) + "_loss_" + str(
        round(runningLoss / (trainset_len_autoencoder / BatchSize), 6)) + ".pth"
    torch.save({
        'epoch': epoch + 1,
        'model_state_dict': net.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss': runningLoss / (trainset_len_autoencoder / BatchSize)}, model_path)
